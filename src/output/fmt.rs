...
pub fn format_size(bytes: i64) -> String {
    ...
    let b = bytes as f64;
    if b >= TB { format!("{:.1} TB", b/TB) } else if b >= GB { format!("{:.1} GB", b/GB) }
    else if b >= MB { format!("{:.1} MB", b/MB) } else if b >= KB { format!("{:.1} KB", b/KB) }
...
pub fn format_size_compact(bytes: i64) -> String {
    ...
    let (v, u) = if b >= GB { (b/GB,"GB") } else if b >= MB { (b/MB,"MB") } else if b >= KB { (b/KB,"KB") } else { (b,"B") };
    let s = if bytes < 0 { "-" } else { "+" };
    if u == "B" { format!("{}{}B", s, b as i64) } else { format!("{}{:.0}{}", s, v, u) }
}
