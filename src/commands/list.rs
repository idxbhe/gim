...
#[derive(Serialize)]
struct GameJson { alias: String, title: String, game_dir: String, data_dir: String, added_at: i64, snapshot_count: usize }
...
pub fn run(colorizer: &Colorizer, details: bool, json: bool) -> GResult<()> {
    ...
    if json {
        ...
        for g in &games {
            ...
            out.push(GameJson { alias: g.alias.clone(), title: g.title.clone(), game_dir: g.game_dir.to_string_lossy().into_owned(), data_dir: g.data_dir.to_string_lossy().into_owned(), added_at: g.added_at, snapshot_count: sc });
        }
        println!("{}", serde_json::to_string_pretty(&out)?);
        return Ok(());
    }
    if games.is_empty() { println!("no games tracked — use `gim add <alias> <game_dir>`"); return Ok(()); }
    if details {
        for g in &games {
    ...
    for g in &games {
        println!("{}  {}  `{}`", colorizer.green(&format!("{:<ma$}", g.alias)), format!("{:<mt$}", g.title), g.game_dir.display());
    }
    Ok(())
}
