...
pub fn run(_c: &Colorizer, alias: String, add: Option<String>, remove: Option<String>, list: bool, edit: bool) -> GResult<()> {
    ...
    if let Some(p) = add {
        let mut f = fs::OpenOptions::new().create(true).append(true).open(&pg)?;
        writeln!(f, "{p}")?;
        println!("added pattern \"{p}\" to {alias}/.gignore");
        return Ok(());
    }
    if let Some(p) = remove {
        if !pg.exists() { println!("no .gignore for {alias}"); return Ok(()); }
        let c = fs::read_to_string(&pg)?;
        let kept: Vec<&str> = c.lines().filter(|l| l.trim() != p.trim()).collect();
        fs::write(&pg, kept.join("\n") + "\n")?;
    ...
    if list || (!edit && add.is_none() && remove.is_none()) {
        ...
        println!("{alias} ignore patterns:\n");
        for src in &set.sources { println!("  {}", src.label); for p in &src.patterns { println!("    {p}"); } println!(); }
        return Ok(());
    }
    if edit {
        if !pg.exists() { fs::write(&pg, b"")?; }
        let ed = std::env::var("EDITOR").unwrap_or_else(|_| if cfg!(windows) { "notepad".into() } else { "vi".into() });
        let st = std::process::Command::new(&ed).arg(&pg).status().map_err(|e| GError::Other(format!("cannot launch editor \"{ed}\": {e}")))?;
        if !st.success() { return Err(GError::Other(format!("editor \"{ed}\" exited with {st}"))); }
    }
    Ok(())
}
